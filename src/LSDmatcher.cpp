    ...
    int LSDmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame)
    {
        ...
        for(int i=0; i<lmatches.size(); i++)
        {
            ...
            if(dist_12>nn12_dist_th)
            {
                ...
                KeyLine line_1 = LastFrame.mvKeylinesUn[qdx];
                KeyLine line_2 = CurrentFrame.mvKeylinesUn[tdx];
    ...
    for(int i=0; i<LastFrame.NL; i++)
    {
        ...
        if(pML)
        {
            if(!LastFrame.mvbLineOutlier[i])
            {
                ...
                float radius = th*CurrentFrame.mvScaleFactorsLine[nLastOctave];

                vector<size_t> vIndices2;

                /*vIndices2 = CurrentFrame.GetFeaturesInAreaForLine(pML->mTrackProjX1, pML->mTrackProjY1, pML->mTrackProjX2, pML->mTrackProjY2,
                                     radius, nLastOctave-1, nLastOctave+1);*/
                vIndices2 = CurrentFrame.GetLinesInArea(pML->mTrackProjX1, pML->mTrackProjY1, pML->mTrackProjX2, pML->mTrackProjY2,
...
void LSDmatcher::ComputeThreeMaxima(vector<int>* histo, const int L, int &ind1, int &ind2, int &ind3)
{
    ...
    int LSDmatcher::SearchByProjection(Frame &F, const std::vector<MapLine *> &vpMapLines, const float th)
    {
        ...
        for(size_t iML=0; iML<vpMapLines.size(); iML++)
        {
            ...
            /*vector<size_t> vIndices =
                    F.GetFeaturesInAreaForLine(pML->mTrackProjX1, pML->mTrackProjY1, pML->mTrackProjX2, pML->mTrackProjY2,
                                     r*F.mvScaleFactorsLine[nPredictLevel], nPredictLevel-1, nPredictLevel);*/
            ...
            /*float delta1x = pML->mTrackProjX1-pML->mTrackProjX2;
            float delta1y = pML->mTrackProjY1-pML->mTrackProjY2;
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
            {
                ...
                /*float delta2x = F.mvKeylinesUn[idx].startPointX - F.mvKeylinesUn[idx].endPointX;
                float delta2y = F.mvKeylinesUn[idx].startPointY - F.mvKeylinesUn[idx].endPointY;
                float norm_delta2 = sqrt(delta2x*delta2x + delta2y*delta2y);
                delta2x /= norm_delta2;
                delta2y /= norm_delta2;
                float CosSita = delta1x * delta2x + delta1y * delta2y;
    ...
    void LSDmatcher::FrameBFMatch(cv::Mat ldesc1, cv::Mat ldesc2, vector<int>& LineMatches, float TH)
    {
        ...
        for(int i=0; i<lmatches.size(); i++)
        {
            ...
            double dist_12 = lmatches[i][1].distance - lmatches[i][0].distance;
            if(dist_12>nn12_dist_th && lmatches[i][1].distance < TH && lmatches[i][0].distance < mfNNratio * lmatches[i][1].distance)
                LineMatches[qdx] = tdx;
        }
    }
    ...
    int LSDmatcher::Fuse(KeyFrame *pKF, const vector<MapLine *> &vpMapLines, float th)
    {
        ...
        for(int i=0; i<nMLs; i++)
        {
            ...
            for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
            {
                ...
                /*float delta2x = pKF->mvKeyLines[idx].startPointX - pKF->mvKeyLines[idx].endPointX;
                float delta2y = pKF->mvKeyLines[idx].startPointY - pKF->mvKeyLines[idx].endPointY;
                float norm_delta2 = sqrt(delta2x*delta2x + delta2y*delta2y);
                delta2x /= norm_delta2;
                delta2y /= norm_delta2;
                float CosSita = delta1x * delta2x + delta1y * delta2y;
            ...
            if( bestDist <= TH_LOW )
            {
                ...
                else
                {
                    ...
                    KeyLine tkl = pKF->mvKeyLines[bestIdx];
                    ...
                    cv::Mat Rcw = pKF->GetPose().rowRange(0,3).colRange(0,3);
