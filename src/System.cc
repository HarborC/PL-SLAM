...
//        mptViewer = new thread(&Viewer::Run, mpViewer); //只显示点特征的
        mptViewer = new thread(&Viewer::RunWithLine, mpViewer); //显示点特征和线特征
...
void System::SaveKeyFrameTrajectoryMonoKitti(const string &filename)
{
    cout << endl << "Saving mono KITTI trajectory to " << filename << " ..." << endl;
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        f << setprecision(9) << R.at<float>(0,0) << " " << R.at<float>(0,1)  << " " << R.at<float>(0,2) << " "  << t.at<float>(0) << " " <<
